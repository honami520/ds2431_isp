...
UINT8	CH376ReadBlock( PUINT8 buf )
{
	...
	CH376_WR_CMD_PORT( CMD01_RD_USB_DATA0 );
	s = l = CH376_RD_DAT_PORT( );  														/* 后续数据长度 */
	if ( l ) 
	{
		do 
		{
...
UINT8	CH376WriteReqBlock( PUINT8 buf )
{
	...
	CH376_WR_CMD_PORT( CMD01_WR_REQ_DATA );                                                /* 向内部指定缓冲区写入请求的数据块 */
	s = l = CH376_RD_DAT_PORT( );  														/* 后续数据长度 */
	if ( l ) 
	{
		do 
...
{
	CH376_WR_CMD_PORT( CMD10_WR_HOST_DATA );
	CH376_WR_DAT_PORT( len );  															/* 后续数据长度 */
	if ( len ) 
	{
		do 
		{
...
{
	CH376_WR_CMD_PORT( CMD20_WR_OFS_DATA );
	CH376_WR_DAT_PORT( ofs );  															/* 偏移地址 */
	CH376_WR_DAT_PORT( len );  															/* 数据长度 */
	if ( len ) 
	{
		do 
		{
...
void	CH376SetFileName( PUINT8 name )
{
	...
	CH376_WR_CMD_PORT( CMD01_GET_IC_VER );													/* 获取芯片版本 */
	if (  CH376_RD_DAT_PORT( ) < 0x43 ) 
	{
		temp = CH376ReadVar8( VAR_DISK_STATUS );
		if ( temp < DEF_DISK_READY ) 
		{
			for(i = 0;i < 10;i ++)
			{
...
UINT32	CH376Read32bitDat( void )
{
	...
	c0 = CH376_RD_DAT_PORT( );
	c1 = CH376_RD_DAT_PORT( );
	c2 = CH376_RD_DAT_PORT( );
	c3 = CH376_RD_DAT_PORT( );	
	CH376_CS_HIGH( );
...
UINT8	CH376ReadVar8( UINT8 var ) 
{
	...
	CH376_WR_CMD_PORT( CMD11_READ_VAR8 );                                                   /* 读取指定的8位文件系统变量 */
	CH376_WR_DAT_PORT( var );
	c0 = CH376_RD_DAT_PORT( );
	CH376_CS_HIGH( );	
	return( c0 );
}
...
{
	CH376_WR_CMD_PORT( CMD20_WRITE_VAR8 );                                                 /* 设置指定的8位文件系统变量 */
	CH376_WR_DAT_PORT( var );
	CH376_WR_DAT_PORT( dat );
    CH376_CS_HIGH( );		
}
...
{
	CH376_WR_CMD_PORT( CMD14_READ_VAR32 );
	CH376_WR_DAT_PORT( var );
...
{
	CH376_WR_CMD_PORT( CMD50_WRITE_VAR32 );
	CH376_WR_DAT_PORT( var );
	CH376_WR_DAT_PORT( (UINT8)dat );
	CH376_WR_DAT_PORT( (UINT8)( (UINT16)dat >> 8 ) );
	CH376_WR_DAT_PORT( (UINT8)( dat >> 16 ) );
	CH376_WR_DAT_PORT( (UINT8)( dat >> 24 ) );
	CH376_CS_HIGH( );		
}
...
UINT8	CH376GetIntStatus( void )
{
	...
	CH376_WR_CMD_PORT( CMD01_GET_STATUS );
	s = CH376_RD_DAT_PORT( );
	CH376_CS_HIGH( );	
	return( s );
}
...
{
	return( CH376_DISK_MOUNT() );									/* 初始化磁盘并测试磁盘是否就绪 */
}
...
UINT8	CH376FileCreate( PUINT8 name )
{
	...
	for(i = 0;i < 10;i ++)
	{
		res = CH376SendCmdWaitInt( CMD0H_FILE_CREATE );
...
UINT8	CH376DirCreatePath( PUINT8 PathName )
{
	...
	{  
		...
		CH376_WR_CMD_PORT( CMD14_READ_VAR32 );
		CH376_WR_DAT_PORT( VAR_START_CLUSTER );  											/* 上级目录的起始簇号 */
		
		for ( s = 0; s != 4; s ++ ) 
		{
			ClustBuf[ s ] = CH376_RD_DAT_PORT( );
		}
...
UINT8	CH376FileClose( UINT8 UpdateSz )
{
	...
	for(i = 0;i < 10;i ++)
	{
		res = CH376SendCmdDatWaitInt( CMD1H_FILE_CLOSE, UpdateSz );
		if(res == USB_INT_SUCCESS)
		{
			break;
		}
...
{
	CH376_WR_CMD_PORT( CMD4H_BYTE_LOCATE );
	CH376_WR_DAT_PORT( (UINT8)offset );
	CH376_WR_DAT_PORT( (UINT8)((UINT16)offset>>8) );
	CH376_WR_DAT_PORT( (UINT8)(offset>>16) );
	CH376_WR_DAT_PORT( (UINT8)(offset>>24) );
	CH376_CS_HIGH( );
	return( Wait376Interrupt( ) );
}
...
UINT8	CH376ByteRead( PUINT8 buf, UINT16 ReqCount, PUINT16 RealCount )
{
	...
	CH376_WR_CMD_PORT( CMD2H_BYTE_READ );
	CH376_WR_DAT_PORT( (UINT8)ReqCount );
	CH376_WR_DAT_PORT( (UINT8)(ReqCount>>8) );
	CH376_CS_HIGH( );
	if ( RealCount ) 
	{
	    *RealCount = 0;
	...
	while ( 1 ) 
	{
		...
		if ( s == USB_INT_DISK_READ )                                                   /* 请求数据读出 */
		{
			...
			CH376_WR_CMD_PORT( CMD0H_BYTE_RD_GO );                                         /* 继续读 */
			CH376_CS_HIGH( );
			buf += s;
			if ( RealCount ) 
			{
...
UINT8	CH376ByteWrite( PUINT8 buf, UINT16 ReqCount, PUINT16 RealCount )
{
	...
	CH376_WR_CMD_PORT( CMD2H_BYTE_WRITE );
	CH376_WR_DAT_PORT( (UINT8)ReqCount );
	CH376_WR_DAT_PORT( (UINT8)(ReqCount>>8) );
	CH376_CS_HIGH( );
	if ( RealCount ) 
	...
	while(1)
	{
		...
		if ( s == USB_INT_DISK_WRITE ) 
		{
			...
			CH376_WR_CMD_PORT( CMD0H_BYTE_WR_GO );
			CH376_CS_HIGH( );
			buf += s;
...
UINT8	CH376DiskCapacity( PUINT32 DiskCap )
{
	...
	{                                                                                   /* 参考CH376INC.H文件中CH376_CMD_DATA结构的DiskCapacity */
		CH376_WR_CMD_PORT( CMD01_RD_USB_DATA0 );
		CH376_RD_DAT_PORT( );                                                              /* 长度总是sizeof(CH376_CMD_DATA.DiskCapacity) */
...
UINT8   CH376DiskQuery( PUINT32 DiskFre )
{
...
#ifndef	DEF_IC_V43_U
	CH376_WR_CMD_PORT( CMD01_GET_IC_VER );                                                 /* 获取芯片及固件版本 */
	if ( CH376_RD_DAT_PORT( ) < 0x43 ) 
	{
	...
	{                                                                                   /* 参考CH376INC.H文件中CH376_CMD_DATA结构的DiskQuery */
		CH376_WR_CMD_PORT( CMD01_RD_USB_DATA0 );
		CH376_RD_DAT_PORT( );                                                              /* 长度总是sizeof(CH376_CMD_DATA.DiskQuery) */
		CH376_RD_DAT_PORT( );                                                              /* CH376_CMD_DATA.DiskQuery.mTotalSector */
		CH376_RD_DAT_PORT( );
		CH376_RD_DAT_PORT( );
		CH376_RD_DAT_PORT( );
		c0 = CH376_RD_DAT_PORT( );                                                         /* CH376_CMD_DATA.DiskQuery.mFreeSector */
		c1 = CH376_RD_DAT_PORT( );
		c2 = CH376_RD_DAT_PORT( );
		c3 = CH376_RD_DAT_PORT( );
		*DiskFre = c0 | (UINT16)c1 << 8 | (UINT32)c2 << 16 | (UINT32)c3 << 24;
		CH376_RD_DAT_PORT( );                                                              /* CH376_CMD_DATA.DiskQuery.mDiskFat */		
		CH376_CS_HIGH( );
	}
	else 
	{
...
{
	CH376_WR_CMD_PORT( CMD4H_SEC_LOCATE );
	CH376_WR_DAT_PORT( (UINT8)offset );
	CH376_WR_DAT_PORT( (UINT8)((UINT16)offset>>8) );
	CH376_WR_DAT_PORT( (UINT8)(offset>>16) );
	CH376_WR_DAT_PORT( 0 );                                                               /* 超出最大文件尺寸 */
	CH376_CS_HIGH( );
	return( Wait376Interrupt( ) );
}
...
UINT8	CH376DiskReadSec( PUINT8 buf, UINT32 iLbaStart, UINT8 iSectorCount )
{
	...
	{  
		CH376_WR_CMD_PORT( CMD5H_DISK_READ );                                              /* 从USB存储器读扇区 */
		CH376_WR_DAT_PORT( (UINT8)iLbaStart );                                            /* LBA的最低8位 */
		CH376_WR_DAT_PORT( (UINT8)( (UINT16)iLbaStart >> 8 ) );
		CH376_WR_DAT_PORT( (UINT8)( iLbaStart >> 16 ) );
		CH376_WR_DAT_PORT( (UINT8)( iLbaStart >> 24 ) );                                  /* LBA的最高8位 */
		CH376_WR_DAT_PORT( iSectorCount );                                                /* 扇区数 */
        CH376_CS_HIGH( );
		...
		{  
			...
			{  
				s = CH376ReadBlock( buf );                                              /* 从当前主机端点的接收缓冲区读取数据块,返回长度 */
				CH376_WR_CMD_PORT( CMD0H_DISK_RD_GO );                                     /* 继续执行USB存储器的读操作 */
				CH376_CS_HIGH( );
				buf += s;
			}
			else 
...
UINT8	CH376DiskWriteSec( PUINT8 buf, UINT32 iLbaStart, UINT8 iSectorCount )
{
	...
	for ( err = 0; err != 3; ++ err )                                                   /* 出错重试 */
	{  
		CH376_WR_CMD_PORT( CMD5H_DISK_WRITE );                                             /* 向USB存储器写扇区 */
		CH376_WR_DAT_PORT( (UINT8)iLbaStart );                                            /* LBA的最低8位 */
		CH376_WR_DAT_PORT( (UINT8)( (UINT16)iLbaStart >> 8 ) );
		CH376_WR_DAT_PORT( (UINT8)( iLbaStart >> 16 ) );
		CH376_WR_DAT_PORT( (UINT8)( iLbaStart >> 24 ) );                                  /* LBA的最高8位 */
		...
		{  
			...
			{  
				...
				CH376_WR_CMD_PORT( CMD0H_DISK_WR_GO );                                     /* 继续执行USB存储器的写操作 */
				CH376_CS_HIGH( );
...
UINT8	CH376SecRead( PUINT8 buf, UINT8 ReqCount, PUINT8 RealCount )
{
	...
	do 
	{
		...
		cnt = CH376_RD_DAT_PORT( );
		
		if ( cnt == 0x41 ) 
		{
			CH376_WR_CMD_PORT( CMD14_READ_VAR32 );
			CH376_WR_DAT_PORT( VAR_FILE_SIZE );
			CH376_RD_DAT_PORT( );
			fsz = CH376_RD_DAT_PORT( );
			fsz |= (UINT16)(CH376_RD_DAT_PORT( )) << 8;
			cnt = CH376_RD_DAT_PORT( );
			fsz |= (UINT32)cnt << 16;
			
			CH376_WR_CMD_PORT( CMD14_READ_VAR32 );
			CH376_WR_DAT_PORT( VAR_CURRENT_OFFSET );
			CH376_RD_DAT_PORT( );
			fofs = CH376_RD_DAT_PORT( );
			fofs |= (UINT16)(CH376_RD_DAT_PORT( )) << 8;
			fofs |= (UINT32)(CH376_RD_DAT_PORT( )) << 16;
...
#endif
		CH376_WR_CMD_PORT( CMD1H_SEC_READ );
		CH376_WR_DAT_PORT( ReqCount );
		CH376_CS_HIGH( );
		s = Wait376Interrupt( );
#ifndef	DEF_IC_V43_U
		if ( cnt != 0xFF ) 
		...
		CH376_WR_CMD_PORT( CMD01_RD_USB_DATA0 );
		CH376_RD_DAT_PORT( );                                                              /* 长度总是sizeof(CH376_CMD_DATA.SectorRead) */
		cnt = CH376_RD_DAT_PORT( );                                                        /* CH376_CMD_DATA.SectorRead.mSectorCount */
		CH376_RD_DAT_PORT( );
		CH376_RD_DAT_PORT( );
		CH376_RD_DAT_PORT( );
...
UINT8	CH376SecWrite( PUINT8 buf, UINT8 ReqCount, PUINT8 RealCount )
{
	...
	do 
	{
		CH376_WR_CMD_PORT( CMD1H_SEC_WRITE );
		CH376_WR_DAT_PORT( ReqCount );
		CH376_CS_HIGH( );
		s = Wait376Interrupt( );
		if ( s != USB_INT_SUCCESS ) 
		...
		CH376_WR_CMD_PORT( CMD01_RD_USB_DATA0 );
		CH376_RD_DAT_PORT( );                                                              /* 长度总是sizeof(CH376_CMD_DATA.SectorWrite) */
		cnt = CH376_RD_DAT_PORT( );                                                        /* CH376_CMD_DATA.SectorWrite.mSectorCount */
		CH376_RD_DAT_PORT( );
		CH376_RD_DAT_PORT( );
		CH376_RD_DAT_PORT( );
...
UINT8	CH376LongNameWrite( PUINT8 buf, UINT16 ReqCount )
{
	...
	while ( 1 ) 
	{
		...
		if ( s == USB_INT_DISK_WRITE ) 
		{
			...
			{
				CH376_WR_CMD_PORT( CMD01_WR_REQ_DATA );                                    /* 向内部指定缓冲区写入请求的数据块 */
				s = CH376_RD_DAT_PORT( );                                                  /* 长度 */
				while ( s -- ) 
				{
				    CH376_WR_DAT_PORT( 0 );                                               /* 填充0 */
				}
			}
...
UINT8	CH376LocateInUpDir( PUINT8 PathName ) 
{
	...
	CH376_WR_CMD_PORT( CMD14_READ_VAR32 );
	CH376_WR_DAT_PORT( VAR_FAT_DIR_LBA );                                                 /* 当前文件目录信息所在的扇区LBA地址 */
	for ( s = 4; s != 8; s ++ ) 
	{
	    GlobalBuf[ s ] = CH376_RD_DAT_PORT( );                                             /* 临时保存于全局缓冲区中,节约RAM */
	}
	...
	*(PUINT32)( &GlobalBuf[0] ) = 0;                                                    /* 目录扇区偏移扇区数,保存在全局缓冲区中,节约RAM */
	...
	{  
		...
		CH376_WR_CMD_PORT( CMD50_WRITE_VAR32 );
		CH376_WR_DAT_PORT( VAR_FAT_DIR_LBA );                                             /* 得到前一个扇区,设置为新的文件目录信息扇区LBA地址 */
		for ( s = 8; s != 12; s ++ ) 
		{
		    CH376_WR_DAT_PORT( GlobalBuf[ s ] );
		}
		CH376_CS_HIGH( );
		++ *(PUINT32)(&GlobalBuf[0]);
	}
